...
- pco_y, pco_x:
  ...
  number; PCO score indices for the y- and x-axes, respectively. `pco_x`
  can be `NULL`.
...
- ...:
  ...
  when `pco_x` is not `NULL`. If `scores` is supplied as an argument, it
  will replace `pco_y` if unspecified.
...
When `pco_x` is `NULL` (the default),
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) will display a
scatterplot of the PCO axis identified by `pco_y` and vertebra position
using
