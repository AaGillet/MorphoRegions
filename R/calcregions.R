...
#'
#' When the number of possible combinations of breakpoints for a given number of regions is larger than 50 million (i.e., 5e7), the problem will be split into smaller problems, with the results of each stored in temporary files that are deleted when the function completes.
#'
...
calcregions <- function(pco, scores, noregions, minvert = 3, cont = TRUE,
  ...
  if (!is.null(includebp)) {
    ...
    if (length(includebp) > 1) {
      ...
      if (any(diff(c(eligible_vertebrae[1] - 1, includebp, max(eligible_vertebrae))) < minvert)) {
        chk::wrn("the breakpoints specified in `includebp` do not obey the `minvert` crtierion")
      }
    }
  }
  ...
  ncombs <- vapply(eligible_noregions - 1, function(i) {
    .ncombos(eligible_vertebrae, minvert, i)
  }, numeric(1L))
...
#' @export
#' @rdname calcregions
addregions <- function(regions_results, noregions, exhaus = TRUE, cl = NULL, verbose = TRUE) {
  # Argument checks
...
.addregions_internal <- function(regions_results, noregions, exhaus = FALSE, cl = NULL, verbose = TRUE) {
  ...
  if (nbp == 0) {
    ...
    if (ncombi <= max_rows) {
      ...
      max_rows <- lim/100 # Max number of rows per iteration; keeps sizes small and
