...
The current development version of *MorphoRegions* (available on GitHub) includes new functionality to allow users to input **principal component (PC) scores computed externally**, including support for **geometric morphometric data**, via the newly added functions `process_PC()` and `process_gmPC()`.

See section below to install the most recent version of the package. Note that this is a development version whic may chnage before next CRAN release.
...
Or the development version (0.2.0.9000), including support for geometric morphometric data, from the `developGMM` branch on [GitHub](https://github.com/AaGillet/MorphoRegions) with:
